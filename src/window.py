...
class MainWindow(QMainWindow):
    ...
    def fill_page(self, where, data):
        ...
        self.current_ayat_index=0
        self.sellected_ayat=self.all_ayat[0]
        self.ayat_list=[]
        ayat_i=0
        for a in self.all_ayat:
            ayat = Ayat_label(str(a[2]),a,ayat_i)
    ...
    def Ayat_clicked(self,info,index):
        self.sellected_ayat=info
        self.current_ayat_index=index
        self.ayat_moving()
        self.play_status="none"
        pl.stopAudio()

    def ayat_moving(self):
        if self.current_ayat_index == len(self.ayat_list):
            self.btn_clk_page_next()
        for a in self.ayat_list:
            a.setStyleSheet("QLabel { color: white; }")
        self.ayat_list[self.current_ayat_index].setStyleSheet("QLabel { color: red; }")
        self.current_ayat_index+=1
    ...
    def btn_clk_play(self):
        ...
        if self.play_status=="none":
            ...
            pl.playAudio((self.sellected_ayat[1],self.sellected_ayat[3],self.sellected_ayat[5]))
